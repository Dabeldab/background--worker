...
async def render_podcast(body: PodcastRenderRequest):
    ...
    try:
        ...
        # This keeps older workflows working, but it does not preserve turn order.
            ...
            SPEAKER_ORDER = ["SPEAKER_1"] if single_speaker else ["SPEAKER_1", "SPEAKER_2", "SPEAKER_3", "SPEAKER_4", "SPEAKER_GIRL"]
            spk_entries = [
        ...
        # ── Build filter_complex (video + optional waveform + subtitles) ───
        video_filter = (
        ...
        if viz_style in ("waveform", "spectrum"):
            if viz_style == "spectrum":
                viz = (f"[1:a]asplit=2[a_vis][a_enc];"
                       f"[a_vis]showspectrum=s={W}x{wave_h}:mode=combined:slide=scroll:color=rainbow:scale=cbrt[viz]")
            else:
                viz = (f"[1:a]asplit=2[a_vis][a_enc];"
                       f"[a_vis]showwaves=s={W}x{wave_h}:mode=cline:colors=0xE8A838|0xE8A838:rate=30[viz]")
        ...
        if srt_path:
            fs = _srt_force_style(body.caption_style)
            srt_e = str(srt_path).replace("\\", "/").replace(":", "\\:")
            filter_parts.append(f"[{current_video}]subtitles='{srt_e}':force_style='{fs}'[vout]")
            current_video = "vout"
        else:
